...
import {
  ...
  Restaurant as RestaurantIcon  // ← AGREGAR ESTA LÍNEA
...
const Reservas = () => {
  ...
  const generarFactura = async (reservaId) => {
    if (!window.confirm('¿Generar factura para esta reserva?')) {
      return;
    }
    ...
    try {
      const response = await api.post(`/facturas/generar-desde-reserva/${reservaId}`, {
        impuestos: 10,
        descuentos: 0,
                ...
                <TableRow key={reserva.id} hover>
                  ...
                  <TableCell align="right">
                    ...
                    {(reserva.estado === 'confirmada' || reserva.estado === 'check_in') && (
                      <Tooltip title="Agregar servicios">
                        <IconButton 
                          size="small" 
                          color="primary"
                    ...
                    {reserva.estado === 'check_out' && (
                      <Tooltip title="Generar Factura">
                        <IconButton 
                          size="small" 
                          color="primary"
                          onClick={() => generarFactura(reserva.id)}
                        >      
                          <ReceiptIcon />
                        </IconButton>
                      </Tooltip>
